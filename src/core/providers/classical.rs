...
impl CryptoProvider for VodoCrypto {
    ...
    /// Estima overhead de mensagem criptografada clássica
    /// 
    /// Converte tamanho Base64 para bytes aproximados:
    /// - Base64 usa 4 caracteres para representar 3 bytes
    /// - Fórmula: bytes ≈ (base64_len × 3) / 4
    /// 
    /// OVERHEAD TÍPICO:
    /// - AES-256-CBC padding: até 16 bytes (bloco AES)
    /// - HMAC-SHA-256: 32 bytes (ou 8 bytes se truncado)
    /// - Metadata (version, index, etc): ~3-8 bytes
    /// - Total overhead: ~20-56 bytes dependendo do tipo de mensagem
    fn estimate_message_overhead(&self, ciphertext_b64: &str) -> usize {
        (ciphertext_b64.len() * 3) / 4
    }
    
    /// Obtém estatísticas detalhadas de PQC (sempre vazio para provider clássico)
    /// 
    /// Provider clássico não realiza operações pós-quânticas (KEM, PQXDH, etc).
    /// Retorna estrutura vazia para manter compatibilidade com interface CryptoProvider.
    fn detailed_pqc_stats(&self) -> DetailedPqcStats {
        DetailedPqcStats::default()
    }
    
    /// Criptografa mensagem Olm com instrumentação de performance (versão clássica)
    /// 
    /// MÉTRICAS COLETADAS:
    /// - total_time_ms: Tempo total de criptografia (AES + serialização + base64)
    /// - kem_time_ms: 0.0 (modo clássico não usa KEM)
    /// - serialization_time_ms: ~0.05ms (overhead de formato Olm + base64)
    /// - ratchet_advanced: false (encrypt não avança ratchet, apenas usa chain_key atual)
    /// - message_overhead_bytes: ciphertext_size - plaintext_size
    /// - kem_algorithm: None (sem operações pós-quânticas)
    /// 
    /// Útil para comparações de performance Classical vs. Hybrid nos experimentos.
    fn olm_encrypt_instrumented(&mut self, session: &mut OlmSessionHandle, plaintext: &[u8]) -> (String, PqcOperationStats) {
        use std::time::Instant;
        
        let start = Instant::now();
        let result = self.olm_encrypt(session, plaintext);
        let elapsed_ms = start.elapsed().as_secs_f64() * 1000.0;
        
        let stats = PqcOperationStats {
            total_time_ms: elapsed_ms,
            kem_time_ms: 0.0,
            serialization_time_ms: 0.05,
            ratchet_advanced: false,
            message_overhead_bytes: result.len().saturating_sub(plaintext.len()),
            kem_algorithm: None,
        };
